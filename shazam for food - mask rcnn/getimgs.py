...
class GetImgs:
    ...
    def is_image_corrupted(image_path):
        try:
            ...
            if file_name[0] == '.': return True  # Ignore hidden files;
    ...
    def _verify_integrity(self, directory: str):
        ...
        if num_images > initial_num:
            for image in os.listdir(directory)[initial_num:]:
    ...
    def _multiple(self):
        for object in self.object:
            try:
                # futures = []
                # with ThreadPoolExecutor() as executor:
                #     future = executor.submit(GetImgs(object, num=self.num, dir=self.dir, overwrite=self.overwrite)._single())
                #     futures.append(future)
                GetImgs(object, num=self.num, dir=self.dir, overwrite=self.overwrite)._single()
                # for future in futures:
                #     future.result()
            except Exception as e:
                print(f'Error in downloading {object}... with exception {e}')
                continue
